...
from keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate, \
                         BatchNormalization, UpSampling2D, Multiply, ELU, Conv2DTranspose, Layer
...
def _deconv_module(feature_layer, deconv_layer, name, feature_size=512):
    ...
    deconv1 = Lambda(lambda args: K.conv2d_transpose(args[0],
                                 ...
                                 (2,2),
                                 'same'))([deconv_layer, feature_layer, deconv_filter])
...
def load_backbone(inputs=None, freeze=True, outputs_names = ['activation_10', 'activation_22', 'activation_40', 'activation_49']):
    ...
    outputs = [resnet50.get_layer(l).output for l in outputs_names]

    return resnet50, outputs


def build_dssd_resnet_model(image_size,
                n_classes,
                mode='training',
    ...
    backbone, outputs = load_backbone(inputs=x1)

    ### Build the deconvolutional blocks
    deconv1 = outputs[3]
    deconv2 = _deconv_module(outputs[2], deconv1, 'D2')
    deconv3 = _deconv_module(outputs[1], deconv2, 'D3')
    deconv4 = _deconv_module(outputs[0], deconv3, 'D4')
    ...
    classes4 = Conv2D(n_boxes[0] * n_classes, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='classes4')(deconv4)
    classes5 = Conv2D(n_boxes[1] * n_classes, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='classes5')(deconv3)
    classes6 = Conv2D(n_boxes[2] * n_classes, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='classes6')(deconv2)
    classes7 = Conv2D(n_boxes[3] * n_classes, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='classes7')(deconv1)
    # Output shape of `boxes`: `(batch, height, width, n_boxes * 4)`
    boxes4 = Conv2D(n_boxes[0] * 4, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='boxes4')(deconv4)
    boxes5 = Conv2D(n_boxes[1] * 4, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='boxes5')(deconv3)
    boxes6 = Conv2D(n_boxes[2] * 4, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='boxes6')(deconv2)
    boxes7 = Conv2D(n_boxes[3] * 4, (3, 3), strides=(1, 1), padding="same", kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='boxes7')(deconv1)
...
def main():
    ...
    model_dssd = build_dssd_resnet_model(image_size=(img_height, img_width, img_channels),
                        n_classes=n_classes,
                        mode='training',
